...
func (cone *Cone) Intersect(r primitives.Ray) []float64 {
	hits := []float64{}
...
func (cone *Cone) Normal(worldPoint primitives.PV) primitives.PV {
	...
	objectPoint := worldPoint.Transform(cone.Inverse())
	...
	worldNormal := objectNormal.Transform(cone.Inverse().Transpose())
	worldNormal.W = 0.0
...
func (cone *Cone) UVMapping(point primitives.PV) primitives.PV {
	objectPoint := point.Transform(cone.Inverse())
